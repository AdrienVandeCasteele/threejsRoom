...
// Clouds
planet.clouds = {}
planet.clouds.geometry = new THREE.SphereGeometry(1.02, 46, 46)
planet.clouds.material = new THREE.MeshStandardMaterial({
    alphaMap: textures.cloudsAlpha,
    bumpMap: textures.cloudsAlpha,
    bumpScale: 0.005,
    transparent: true
})
...
for(let i = 0; i < 100000; i++)
{
    ...
    point.x += Math.random()*0.2
    point.y += Math.random()*0.2
    point.z += Math.random()*0.2
...
const loop = () =>
{
    ...
    planet.container.rotation.y = time * 0.00015
    planet.clouds.mesh.rotation.y = - time * 0.00002
    planet.ring.points.rotation.y = - time * 0.00100

    camera.position.x = Math.cos(mouse.x * 3) * 3
    camera.position.z = Math.sin(mouse.x * 3) * 3
